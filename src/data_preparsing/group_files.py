...
def window(
    medatada: dict, width: int, sliding: bool = False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "metadata", help="Path to the metadata file", type=argparse.FileType("r")
    )
    parser.add_argument(
        "--root",
        help="Path to root fs. Defaults to using the metadata dir",
        default=None,
    )

    subparsers = parser.add_subparsers(
        help="parsing methods", required=True, dest="method"
    )

    parser_window = subparsers.add_parser(
        "window", help="Return a series of windows of a certain size"
    )
    parser_window.add_argument("size", type=int, help="size of window")
    parser_window.add_argument(
        "--sliding",
        action="store_true",
        help="If specified, makes the window a sliding window",
    )

    parser_single = subparsers.add_parser("single", help="Outputs one year at a time")

    parser_bulk = subparsers.add_parser(
        "bulk", help="Return all or a slice of all years"
    )
    parser_bulk.add_argument(
        "--slice", help="Years to slice, e.g. 2012-2015. Use - as a separator."
    )
    ...
    match args.method:
        ...
        case "bulk":
            if args.slice:
                ...
                write_output((slice(metadata, start=min(years), end=max(years)),))
            else:
